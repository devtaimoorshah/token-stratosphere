...
# and build "/code/contracts/mycontract". The default value for $1 is "." (see CMD in the Dockerfile).
...
# This will be used later in the optimization step
TARGET_DIR="/home/huzaifakhan/main/Stratocoin/token-stratosphere/target/wasm32-unknown-unknown/release"
ARTIFACTS_DIR="/home/huzaifakhan/main/Stratocoin/token-stratosphere/artifacts"
...
for WASM in "$TARGET_DIR"/*.wasm; do
  [ -e "$WASM" ] || continue # Skip if no .wasm file exists

  OUT_FILENAME=$(basename "$WASM")
  echo "Optimizing $OUT_FILENAME ..."
  wasm-opt -Os --signext-lowering "$WASM" -o "$ARTIFACTS_DIR/$OUT_FILENAME"
done

echo "Post-processing artifacts..."

# Post-process the optimized artifacts
(
  cd "$ARTIFACTS_DIR"
